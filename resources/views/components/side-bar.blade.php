 ...
 <ul class="navbar-nav bg-gradient-info sidebar sidebar-dark accordion" id="accordionSidebar">
     ...
     @hasrole('admin')
         <li class="nav-item {{ request()->is('admin/master_data*') ? 'active' : '' }}">
             ...
             <div id="collapseMasterData" class="collapse {{ request()->is('admin/master_data*') ? 'show' : '' }}"
                 aria-labelledby="headingMasterData" data-parent="#accordionSidebar">
                 <div class="py-2 bg-white rounded collapse-inner">
                     <h6 class="collapse-header">Data Management:</h6>
                     {{-- @can('menu.view') --}}
                     <a class="collapse-item {{ request()->is('admin/master_data/menu*') ? 'active' : '' }}"
                         href="{{ route('admin.master_data.menu.index') }}">Menu</a>
                     {{-- @endcan               --}}
                     <a class="collapse-item {{ request()->is('admin/master_data/user*') ? 'active' : '' }}"
                         href="{{ route('admin.master_data.user.index') }}">User</a>
                     <a class="collapse-item {{ request()->is('admin/master_data/role*') || request()->is('admin/master_data/permission*') ? 'active' : '' }}"
                         href="{{ route('admin.master_data.role.index') }}">Role & Permission</a> <!-- Menambahkan Role -->
